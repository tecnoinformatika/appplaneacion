    ...
    */
    ...
    function createMarkers(){
        ...
        for (var i = 0; i < loadedMarkersData.length; i++) {
            ...
            ( ( loadedMarkersData[i]["NomSede"] !== undefined ) ? '<div class="ts-marker__title">' + loadedMarkersData[i]["NomSede"] + '</div>' : "" ) +
            ( ( loadedMarkersData[i]["NomEstable"] !== undefined && loadedMarkersData[i]["NomEstable"] > 0 ) ? '<div class="ts-marker__info">' + loadedMarkersData[i]["NomEstable"] + '</div>' : "" ) +
            ( ( loadedMarkersData[i]["Tipo"] !== undefined ) ? '<div class="ts-marker__image ts-black-gradient" style="background-image: url(assets/img/colegio.png)"></div>' : '<div class="ts-marker__image ts-black-gradient" style="background-image: url(assets/img/colegio.png)"></div>' ) +
            '</a>';
    ...
    function placeRichMarker(parameters){
        ...
        marker = new RichMarker({
            position: new google.maps.LatLng( loadedMarkersData[i]["X"], loadedMarkersData[i]["Y"] ),
            map: map,
            draggable: false,
        ...
        google.maps.event.addListener(marker, 'click', (function(marker, i) {
            return function() {
                ...
                openInfobox({"id": $(this.content.firstChild).attr("data-ts-id"), "parentMarker": this, "i": i, "url": "/instituciones" });
    ...
    function createInfoBoxHTML(parameters){
        ...
        */
        var id = '';
        $.ajax({
            url: '/cupos/'+ loadedMarkersData[i]["CodSede"],
            type: 'get',
            dataType: 'json',
            success:function(response){
            
            var len = response.length;
            console.log(loadedMarkersData[i]);
            id = loadedMarkersData[i]["id"];
            for( var i = 0; i<len; i++){
              var grado = response[i]['grado'];
              var cupos = response[i]['cupos'];
              
              $("#badge"+id).append("<b>"+grado+": "+cupos+"</b>");
        ...
        '<div class="ts-infobox" data-ts-id="'+ loadedMarkersData[i]["id"] +'">' +
            ...
            ( ( loadedMarkersData[i]["ribbon"] !== undefined ) ? '<div class="ts-ribbon">'+ loadedMarkersData[i]["ribbon"] +'</div>' : "<div class='ts-ribbon'><i class='fa fa-thumbs-up'></i></div>" ) +
            ( ( loadedMarkersData[i]["ribbon_corner"] !== undefined ) ? '<div class="ts-ribbon-corner"><span>'+ loadedMarkersData[i]["ribbon_corner"] +'</span></div>' : "" ) +

            '<a href="/detalle/'+ loadedMarkersData[i]["id"] +'" class="ts-infobox__wrapper ts-black-gradient">' +
                '<div class="badge badge-dark" id="badge'+ loadedMarkersData[i]["id"] +'">Cupos disponibles</div>' +
                '<div class="ts-infobox__content">' +
                    '<figure class="ts-item__info">' +
                        ( ( loadedMarkersData[i]["Zona"] !== undefined ) ? '<div class="ts-item__info-badge">'+ loadedMarkersData[i]["Zona"] +'</div>' : "" ) +
                        ( ( loadedMarkersData[i]["NomSede"] !== undefined && loadedMarkersData[i]["NomSede"].length > 0 ) ? '<h4>'+ loadedMarkersData[i]["NomSede"] +'</h4>' : "" ) +
                        ( ( loadedMarkersData[i]["Direccion"] !== undefined && loadedMarkersData[i]["Direccion"].length > 0 ) ? '<aside><i class="fa fa-map-marker mr-2"></i>'+ loadedMarkersData[i]["Direccion"] +'</aside>' : "" ) +
                    '</figure>' +
                ...
                '</div>' +
                '<div class="ts-infobox_image" style="background-image: url(https://planeacion.novatics.co/storage/'+ loadedMarkersData[i]["imagen"]+ ')"></div>' +
            '</a>' +
        '</div>';
    }
    ...
    function createSideBarResult(){
        ...
        $('#total').html(visibleMarkersOnMap.length + ' Instituciones con cupos disponibles');
        for( i = 0; i < visibleMarkersOnMap.length; i++ ){
            ...
            resultsHtml.push(
                '<div class="ts-result-link" data-ts-id="' + loadedMarkersData[id]["id"] + '" data-ts-ln="' + newMarkers[id].loopNumber + '">' +
                    ...
                    '<a href="/detalle/'+ loadedMarkersData[id]["id"] +'" class="card ts-item ts-card ts-result">' +
                        ( ( loadedMarkersData[i]["ribbon"] !== undefined ) ? '<div class="ts-ribbon">'+ loadedMarkersData[i]["ribbon"] +'</div>' : "<div class='ts-ribbon'><i class='fa fa-thumbs-up'></i></div>" ) +
                        ( ( loadedMarkersData[i]["ribbon_corner"] !== undefined ) ? '<div class="ts-ribbon-corner"><span>'+ loadedMarkersData[i]["ribbon_corner"] +'</span></div>' : "" ) +
                        '<div href="detail-01.html" class="card-img ts-item__image" style="background-image: url(https://planeacion.novatics.co/storage/'+ loadedMarkersData[i]["imagen"]+ ')"></div>' +
                        '<div class="card-body">' +
                            
                            '<figure class="ts-item__info">' +
                                '<h4>'+ loadedMarkersData[id]["NomSede"] +'</h4>' +
                                '<aside>' +
                                    '<i class="fa fa-map-marker mr-2"></i>'+ loadedMarkersData[id]["Direccion"] +'</aside>' +
                            '</figure>' +
                            additionalInfoHTML({display: displayAdditionalInfo, i: i}) +
                        '</div>' +
                        '<div class="card-footer">' +
                            '<span class="ts-btn-arrow">Detalle</span>' +
                        '</div>' +
                    '</a>' +
                '</div>'
            );
